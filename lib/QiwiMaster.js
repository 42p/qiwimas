...
class QiwiMaster {
    ...
    createRequest() {
        this.requestOptions = {
            jar: request.jar(new CookieStore(path.join(__dirname, '../storage/' + this.qiwiAuth.phone.substring(1) ))),
            headers: {
    ...
    parseTransactions(html) {
        ...
        var transcations = $('.reportsLine').map((i, elem) => {
            ...
            return {
                ...
                commission: el.find('.expenditure .commission').length ? parseAmount(el.find('.expenditure .commission').text().trim()).split('\n')[0] : false
            };
        }).get();
