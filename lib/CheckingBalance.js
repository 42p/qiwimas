...
class CheckingBalance {
    ...
    updaterTransactions(statusTransactions) {
        return this.qiwiMaster.getHistoryWeek().then(transactions => {
            ...
            return this.getDbConnection().then((conn) => {
                ...
                let updateTransactionProcessing = (conn) => {
                    return conn.query('SELECT * FROM qiwi_transactions WHERE `purse`=? ORDER BY id DESC LIMIT 1', [this.qiwiMaster.currentPurse().substring(1)])
                        .then(results => {
                                ...
                                return new Promise((resolve, reject) => {
                                    if (trans.status && !trans.expenditure) {
                                        ...
                                        let transData = {
                                            ...
                                            purse: this.qiwiMaster.currentPurse().substring(1),
                                        ...
                                        if (statusTransactions) {
                                            transData.status = statusTransactions;
                                        }

                                        this.addTransaction(transData).then(results => {
                        ...
                        if (err.code == 'EPIPE') {
                            ...
                            return this.createMysqlConnection().then(conn => {
                                console.log('Again updates transaction...');

                                return updateTransactionProcessing(conn);
                            });
                        } else {
                            return err;
                        }
