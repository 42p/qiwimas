...
class QiwiMaster {
    ...
    constructor(qiwiAuthData, pathFileCookie) {
        ...
        this.requestOptions = {};
        this.request = null;
        this.httpProxy = qiwiAuthData.proxy || null;
        this.pathFileCookie = pathFileCookie;
    }

    createRequest() {
        let pathFileCookie = this.pathFileCookie ? this.pathFileCookie : process.cwd() + '/storage';

        this.requestOptions = {
            jar: request.jar(new CookieStore(path.join(pathFileCookie, this.qiwiAuth.phone.substring(1) ))),
            headers: {
        ...
        if (this.httpProxy) {
            this.requestOptions.proxy = this.httpProxy;
        }
    ...
    currentPurse() {
        return this.qiwiAuth.phone;
    }

    changePurse(qiwiAuthData = {}) {
        if (!qiwiAuthData.phone || !qiwiAuthData.pass)
            throw new Error('Invalid params qiwiAuthData');
